...
- (IBAction)showFunctionBtnPress:(id)sender
{
    ...
    [SLPMilkyBleManager bleDisconnectPeripheralWithsuccess:nil];//此处断开蓝牙,为了完整演示连接-登录设备过程
...
{
    [SLPMilkyBleManager bleDisconnectPeripheralWithsuccess:nil];
    self.selectedDeviceSuccessView.hidden = YES;
...
{
    [SLPMilkyBleManager.centerManager stopScan];//停止扫描设备
...
{
    __weak typeof(self) weakSelf = self;
    [SLPMilkyBleHelper initWithPeripheral:peripheral readCharactertic:SLPMilkyBleManager.readCharactertic];
    
    [NewBleServiceInterface getMilkyIdWithSuccess:^(NSString *milkyID) {
        selectedDeviceID = milkyID;
